...
exports.createEnderecoCoberto = async (req, res) => {
  const { rua, numero_inicio, numero_fim, bairro, cidade, estado, cep, id_hospital, id_ubs } = req.body;
  ...
  const { rows } = await db.query(
    ...
    [rua, numero_inicio, numero_fim, bairro, cidade, estado, cep, id_hospital, id_ubs]
  );
  
  res.status(201).send({
        enderecocobertura: { rua, numero_inicio, numero_fim, bairro, cidade, estado, cep, id_hospital, id_ubs}
  });
};
...
exports.findEnderecoCobertoById = async (req, res) => {
  ...
  const response = await db.query('SELECT * FROM enderecocobertura WHERE id = $1', [id]);
  res.status(200).send(response.rows);
}
...
exports.updateEnderecoById = async (req, res) => {
  ...
  const response = await db.query(
    ...
    [rua, numero_inicio, numero_fim, bairro, cidade, estado, cep, id_hospital, id_ubs, id]
  );
